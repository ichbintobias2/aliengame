...
public class EnemyBehaviorController extends StateController<Enemy> {
	...
	private boolean isPlayerInRange(int range) {
		return GeometricUtilities.distance(Player.instance().getCollisionBoxCenter(), getEntity().getCollisionBoxCenter()) < range;
	}
	...
	public void update() {
		if (!getEntity().isDead() && !getEntity().getAttackAbility().isActive()) {
			if (isPlayerInRange(getEntity().getAttackAbility().getAttributes().impact().get()) && canAttack() && !Player.instance().isDead()) {
				attack();
